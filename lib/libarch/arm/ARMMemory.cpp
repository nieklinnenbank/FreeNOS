...
ARMMemory::ARMMemory(Address pageDirectory, BitArray *memoryMap)
    : VirtualMemory(pageDirectory, memoryMap)
{
    ...
    if (pageDirectory)
    {
        ...
        // it will need to run in ring0 for that on ARM, which is acceptable.
        if (isKernel)
        {
...
bool ARMMemory::clone(Address pageDirectory)
{
    ARMMemory mem(pageDirectory, m_memoryMap), local(0, m_memoryMap);
    Address phys, virt;
    Access access;
